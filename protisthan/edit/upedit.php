...
include '../../_dbconnect.php';
    ...
    if (isset($_REQUEST['id'])) {
        ...
        $query="SELECT * FROM `upprotisthan` WHERE id=$kid";
...
Back</a>
<p class="text-center font-semibold text-2xl md:text-4xl">ইউনিয়নের প্রতিষ্ঠানের তথ্য এডিট করুণ  </p>

    <?php

        $kid= $result['id'];
        $college= $result['name'];
        $maddhomik= $result['pdhoron'];
        
        
        ?>
          <form class="mt-5 px-5 flex flex-col gap-4 justify-center items-center max-w-2xl mx-auto" action="upupdate.php" method="POST">
          
          <input type="hidden" name="hid" value="<?php echo $kid;?>">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-5 w-full">
          <input name='sname' type="text" class='px-3 py-2 border-gray-300 bg-white w-full rounded-xl text-sky-900' value='<?php echo $college ?>' >
        <input name='sdhoron' type="text" class='px-3 py-2 border-gray-300 bg-white w-full rounded-xl text-sky-900' value='<?php echo $maddhomik ?>' >
